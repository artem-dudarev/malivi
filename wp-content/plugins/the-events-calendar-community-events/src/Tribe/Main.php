	...
	class Tribe__Events__Community__Main {
		...
		public function register_resources() {
			...
			wp_register_style(
				...
				$stylesheet_url,
				array(),
				apply_filters( 'tribe_events_community_css_version', self::VERSION )
			);

			wp_register_script(
				Tribe__Events__Main::POSTTYPE . '-community',
				$this->pluginUrl . 'src/resources/js/tribe-events-community.js',
				array( 'jquery' ),
				apply_filters( 'tribe_events_community_js_version', self::VERSION )
			);
		 ...
		 * @since 1.0
		 */
		public function formCategoryDropdown( $event, $taxonomy_name) {
			// require an admin functions file so we can leverage wp_terms_checklist
			include_once ABSPATH . '/wp-admin/includes/template.php';
			$walker = new Walker_Category_Checklist;
			?>
			<div id="event-taxonomy-<?php echo $taxonomy_name?>">
		...
		public function init() {
			...
			$this->rewriteSlugs['edit']   = sanitize_title( 'edit' );
			$this->rewriteSlugs['add']    = sanitize_title( 'add');
			$this->rewriteSlugs['delete'] = sanitize_title( 'delete');
			$this->rewriteSlugs['list']   = sanitize_title( 'list');

			$this->rewriteSlugs['venue']     = sanitize_title( 'place');
			$this->rewriteSlugs['organizer'] = sanitize_title( 'organizer');
			$this->rewriteSlugs['event']     = sanitize_title( 'event');
